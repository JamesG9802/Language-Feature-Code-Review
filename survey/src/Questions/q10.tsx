...
/**
 * Question 10 - SyntaxSugar+Correct
 */
function Q10() {
...
This modified version of C has a new loop construct known as `}<strong>foreach.</strong>{` It is intended to be a shorthand way of writing for loops to access variables.
So`}</Typography>
<pre>{
`int i = 0;
int A[3] = {0,1,2};
...
void radix_clone(Movie src[], Movie dst[], unsigned int n) {
    ...
    <Question title="The code is readable." isValid={isValid}>
        <LikertScale value={readable} onChange={(event)=>{
            ...
            survey_data["sy-s-c_readable"] = Number(event.target.value) as survey_scale;
        }} />
    </Question>
    <Question title="The code is simple." isValid={isValid}>
        <LikertScale value={simple} onChange={(event)=>{
            setSimple(event.target.value);
            survey_data["sy-s-c_simple"] = Number(event.target.value) as survey_scale;
        }} />
    </Question>
    <Question title="The code is maintainable." isValid={isValid}>
        <LikertScale value={maintain} onChange={(event)=>{
            ...
            survey_data["sy-s-c_maintainable"] = Number(event.target.value) as survey_scale;
        }} />
    </Question>
    <Question title="The code works as expected." isValid={isValid}>
        <LikertScale value={expected} onChange={(event)=>{
            ...
            survey_data["sy-s-c_works"] = Number(event.target.value) as survey_scale;
        }} />
    </Question>
    <Question title="You would approve and accept this code into a codebase." isValid={isValid}>
        <LikertScale value={approved} onChange={(event)=>{
            ...
            survey_data["sy-s-c_approve"] = Number(event.target.value) as survey_scale;
        }} />
    </Question>
    </>
}
